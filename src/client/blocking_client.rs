...
use reqx::{advanced::PermissiveRetryEligibility, blocking::Client as HttpClient};
use url::Url;
...
/// Builder for blocking [`BlockingClient`].
#[derive(Debug, Clone, Default)]
pub struct BlockingClientBuilder {
    config: BuilderConfig,
}

shared::impl_builder_methods!(BlockingClientBuilder);
...
impl BlockingClient {
    ...
    /// Creates a blocking webhook robot service.
    #[must_use]
    pub fn webhook(
        &self,
        token: impl Into<String>,
        secret: Option<String>,
    ) -> BlockingWebhookService {
        BlockingWebhookService::new(self.clone(), token, secret)
    }

    /// Creates a blocking enterprise robot service.
    #[must_use]
    pub fn enterprise(
        &self,
    ...
    ) -> BlockingEnterpriseService {
        BlockingEnterpriseService::new(self.clone(), appkey, appsecret, robot_code)
    }
